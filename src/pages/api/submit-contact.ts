...
const checkRateLimit = (clientIP: string): boolean => {
    ...
    const requestCount = Object.keys(rateLimitStore).filter(
        ip => ip.startsWith(clientIP)
    ).length;
...
export default async function handler(req: ContactApiRequest, res: NextApiResponse<ApiResponse>): Promise<void> {
    ...
    try {
       ...
       const rowData: RawRowData = {
        ...
        return res.status(500).json({
            error: 'Unable to process your request right now. Please try again or contact us directly.'
        });
    }
